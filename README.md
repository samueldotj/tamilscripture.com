...
| 5.10 | Sub-verse highlights and notes: drag-selecting part of a verse (or from part-way through one verse into another) and choosing a colour or Note marks just those words; stored as code-point offsets into the version read (`version`, `char_start`, `char_end`, `quote` on `highlights` and `notes`; pgTAP `partial_verse_marks`), shown as a whole-verse mark in other versions; the words show on `/me/notes` and `/me/highlights`. The reader's own notes sit in the margin beside their words when the column has room, else under the verse ("My notes in the margin" setting, on by default) | R-10.15, R-10.16 | ☑ |
