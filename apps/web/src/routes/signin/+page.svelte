...
<section class="signin card">
	<h1 lang={ta ? 'ta' : 'en'}>{ta ? 'உள்நுழை' : 'Sign in'}</h1>
	<p class="why" lang={ta ? 'ta' : 'en'}>{ta ? 'வாசிப்பதற்கு கணக்கு தேவையில்லை. உள்நுழைந்தால் உங்கள் வாசிப்பு வரலாறு, குறிப்புகள், அடிக்கோடிடல்கள் சாதனங்களுக்கு இடையே சேமிக்கப்படும்.' : 'No account is needed to read. Signing in keeps your history, notes and highlights across devices.'}</p>

	{#if sent}
		<p class="ok" role="status" lang={ta ? 'ta' : 'en'}>{ta ? `${email} க்கு ஒரு இணைப்பு அனுப்பப்பட்டது. அதைத் திறந்தால் உள்நுழைவீர்கள்.` : `We sent a link to ${email}. Open it to finish signing in.`}</p>
	{:else}
		<form onsubmit={magic}>
			<label class="kicker" for="email">{ta ? 'மின்னஞ்சல்' : 'Email'}</label>
			<input class="field" id="email" type="email" bind:value={email} required autocomplete="email" inputmode="email" placeholder="you@example.com" />
...
<style>
	...
	.why { color: var(--ink-2); margin: 0; line-height: 1.65; }
	.why[lang='ta'] { font-family: var(--tamil); }
	form { display: grid; gap: 0.5rem; margin-top: 1.5rem; }
	label { font-family: var(--tamil); }
	input.field { font-family: var(--sans); min-height: 52px; }
	form .chip { min-height: 52px; border-radius: var(--r-l); font-size: 1rem; }
	.or { display: flex; align-items: center; gap: 0.8rem; margin: 1.2rem 0; color: var(--muted); font-size: 0.85rem; font-family: var(--tamil); }
	.or::before, .or::after { content: ''; flex: 1; border-top: var(--bw) solid var(--line); }
	.google { width: 100%; min-height: 52px; border-radius: var(--r-l); font-size: 1rem; }
	.ok { padding: 1rem; border: var(--bw) solid var(--accent); border-radius: var(--r-l); background: var(--accent-soft); line-height: 1.6; }
	.ok[lang='ta'] { font-family: var(--tamil); }
	.err { color: var(--amber); }
	.privacy { margin: 2rem 0 0; font-size: 0.85rem; font-family: var(--tamil); }
</style>
