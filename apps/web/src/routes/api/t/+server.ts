...
import { MAX_BATCH } from '$lib/analytics/track';

export const prerender = false;

// POST /api/t — a visit's analytics events (docs/feature_analytics.md §2). Adds what
...
export const POST: RequestHandler = async ({ request, getClientAddress, url, fetch }) => {
	...
	// A batch of events (the page queues them and sends one request per visit),
	// or a single event from a page loaded before batching.
	let events: Record<string, unknown>[];
	try {
		const text = await request.text();
		if (text.length > 2000 * MAX_BATCH) return done('skipped:size');
		const parsed = JSON.parse(text);
		events = (Array.isArray(parsed) ? parsed : [parsed]).filter((e) => e && typeof e === 'object').slice(0, MAX_BATCH);
	} catch {
	...
	if (rows.length === 0) return done(events.length ? 'skipped:limited-or-path' : 'skipped:body');
	if (dryRun) return done('dry-run');
	// One RPC per event, in order, so each is hashed and capped exactly as before.
