<script lang="ts">
	...
	import { accept, correctDirectly, exportStatus, parseTarget, publishNow, queue, reject, nameTarget, articleTarget, type ExportStatus, type QueueItem, type Role } from '$lib/community/repo';
...
<div class="head">
	<h1 lang={ta ? 'ta' : 'en'}>{ta ? 'மதிப்பாய்வு வரிசை' : 'Review queue'} <span class="n">{items.length}</span></h1>
...
{:else if !items.length}
	<p class="muted" lang={ta ? 'ta' : 'en'}>{ta ? 'வரிசை காலியாக உள்ளது.' : 'The queue is empty.'}</p>
{:else}
	<ul class="queue">
		{#each items as it, i (it.id)}
			{@const p = parseTarget(it.target)}
			{#if i === 0 || groupKey(items[i - 1].target) !== groupKey(it.target)}
				<li class="group">
			...
			<li class="item">
				...
				<div class="cols">
					<div>
						...
						{#if it.reason}<p class="reason"><span class="k" lang={ta ? 'ta' : 'en'}>{ta ? 'காரணம்' : 'Reason'}:</span> {it.reason}</p>{/if}
					</div>
					<div>
...
<style>
	...
	.err { margin: 0; color: var(--red, #b3261e); font-size: 0.9rem; }
	.ok { margin: 0; color: var(--green, #2e7d32); font-size: 0.9rem; word-break: break-all; }
