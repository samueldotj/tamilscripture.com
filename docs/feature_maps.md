...
| Cliopatria (Seshat Global History Databank) | Polity borders with the years they apply to; the atlas timeline, 3400 BCE onward. 1,773 rows and 236 polities inside Europe, the Middle East and Egypt, west of the Indus; added 14 Sep 2026 | CC BY 4.0 (verified) | `data/entities/geo/polities/` |
...
Kingdoms and regions come from Cliopatria rather than being hand-drawn. `scripts/build-polities.py` clips it to lon −12…68, lat 5…62 — Europe, the Middle East and Egypt, stopping at the Indus — and to 4000 BCE – AD 800 — the end is set by the last thing the timeline carries, the second council of Nicaea in 787, not by the kingdoms — simplifies to 0.1°, and writes one `polities.geojson` in which every row carries the `from`/`to` years it applies to, a point and bbox span to hang its name on, and its Wikipedia and Wikidata ids. Because the years are in the data, the explore map fetches the file once and filters it in the browser: the slider has one step per year at which a border actually changes or the early church reaches somewhere new (234 of them), and moving it costs no request. A kingdom with less than 15% of its territory west of the Indus is dropped too, judged over its whole life so that none flickers in and out: the Maurya and Gupta empires reach Gandhara and the Tang held the Tarim basin, and drawing those clipped slivers would label the edge of the map with an empire that is not on it. The file is 3.5 MB, and the map fetches it once the map itself has drawn, so the first paint does not wait on it.
...
The timeline carries the early church too. Every entry has the year it enters the story — a father's birth, the year a council met, and for a see either the apostolic age, when the New Testament names the city, or the first father or council it holds. With the timeline on, the church is shown as it had reached by the chosen year, so from Pentecost onward it arrives city by city: at AD 68 only the six sees the New Testament names, the Jerusalem council and Ignatius. An entry Wikidata cannot date at all — Athenagoras — is left off the timeline and appears only with it off.

On the map, the early church is one pin per city — Rome holds four fathers and a see — with a creed mark where a council met, and a popup listing what the city holds with its dates and Wikipedia links. The sidebar lists the councils by year and the fathers by tradition. Tamil names come from `church-ta.toml`, seeded from Wikidata and drafts until confirmed, the same arrangement as the polities. In both files `keep = true` marks a name corrected here because Wikidata named something else — it stays a draft, but `--fetch-ta` will not overwrite it.
