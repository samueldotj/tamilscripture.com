...
| `votes_up`, `votes_down` | Thumbs up and down from the closing slide. Only `presentation_vote()` moves them (the owner's update grant excludes them); a private presentation takes no votes |
| `created_at`, `updated_at` | |

A slide (`title` is the one-line header across the top; on a slide without verses it is the title page's heading):
...
| Vote stuffing | One vote per browser is all the page enforces; the counters can only move through `presentation_vote()`, never be set, and a private presentation ignores votes. Noise is accepted, as for the analytics |
